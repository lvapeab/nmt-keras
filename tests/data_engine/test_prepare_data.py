...
class TestDataset(unittest.TestCase):
    ...
    def test_build_datset(self):
        ...
        for split in ['train', 'val', 'test']:
            self.assertEqual(eval('ds.len_' + split), eval('len_' + split))
            self.assertTrue(eval('all(ds.loaded_' + split + ')'))
            self.assertEqual(len(eval('ds.X_' + split + str([params['INPUTS_IDS_DATASET'][0]]))), eval('len_' + split))
            self.assertEqual(len(eval('ds.Y_' + split + str([params['OUTPUTS_IDS_DATASET'][0]]))), eval('len_' + split))
    ...
    def test_update_dataset_from_file(self):
        ...
        self.assertIsInstance(ds, Dataset)
        for splits in [[], ['val']]:
            for output_text_filename in [None, params['DATA_ROOT_PATH'] + params['TEXT_FILES']['test'] + params['TRG_LAN']]:
