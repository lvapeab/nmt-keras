...
def test_NMT_models():
    ...
    # Test several model configurations
    for bidirectional_encoder in [True, False]:
        for n_layers in [1, 2]:
            for encoder_type in ['LSTM', 'GRU']:
                for decoder_type in ['LSTM', 'ConditionalLSTM', 'GRU', 'ConditionalGRU']:
                    ...
                    params['BIDIRECTIONAL_ENCODER'] = bidirectional_encoder
                    params['N_LAYERS_ENCODER'] = n_layers
                    params['BIDIRECTIONAL_DEEP_ENCODER'] = bidirectional_encoder
