...
class TranslationModel(Model_Wrapper):
    ...
    def __init__(self, params, model_type='Translation_Model', verbose=1, structure_path=None, weights_path=None,
        ...
        if structure_path:
            ...
            if hasattr(self, model_type):
                if self.verbose > 0:
                    logging.info("<<< Building " + model_type + " Model >>>")
    ...
    def RNNLM(self, params):
        ...
        # 1. Text input
        input_text = Input(name=self.ids_inputs[0], batch_shape=tuple([None, None]))
        # 2. Word embedding
        input_text_word_embedding = Embedding(params['INPUT_VOCABULARY_SIZE'], params['TARGET_TEXT_EMBEDDING_SIZE'],
                                   name='word_embedding',
                                   embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                   embeddings_initializer=params['INIT_FUNCTION'],
                                   trainable=self.trg_embedding_weights_trainable,
        ...
        input_text_word_embedding = Regularize(input_text_word_embedding, params, name='word_embedding')
        input_RNN_LM = [input_text_word_embedding]

        # 3.3. RNN for processing the input text. Get decoder's params. Only LSTM/
        sharedRNNLM = eval(params['LM_RNN_TYPE'])(params['DECODER_HIDDEN_SIZE'],
                                                  kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                  recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                  bias_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                  dropout=params['RECURRENT_DROPOUT_P'],
                                                  ...
                                                  # return_states=True,
                                                  trainable=params.get('TRAINABLE_DECODER', True),
                                                  name='LM_' + params['LM_RNN_TYPE'])
        proj_h = sharedRNNLM(input_RNN_LM)
        ...
        shared_FC_ctx = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='logit_ctx')
        out_layer_ctx = shared_FC_ctx(input_text_word_embedding)
        ...
        shared_additional_output_merge = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='additional_input')
        additional_output = shared_additional_output_merge([out_layer_mlp, out_layer_ctx])
        shared_activation = Activation(params.get('SKIP_VECTORS_SHARED_ACTIVATION', 'tanh'))
        ...
        self.model = Model(inputs=input_text, outputs=softout)

        # if params['OPTIMIZED_SEARCH']:
        #     self.model_init = self.model
        #     self.model_next = self.model
        #     # Store inputs and outputs names for model_next
        #     # first input must be previous word
        #     self.ids_inputs_next = [self.ids_inputs[0]]
        #     # first output must be the output probs.
