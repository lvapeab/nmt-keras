...
from keras_wrapper.dataset import loadDataset, saveDataset, Data_Batch_Generator
...
from keras_wrapper.online_trainer import OnlineTrainer, ReinforceTrainer
from keras_wrapper.cnn_model import saveModel, updateModel


def train_reinforce_model(params, load_dataset=None):
    """
    Training function.
    ...
    nmt_model = build_online_models([nmt_model], params)[0]

    # Training
    total_start_time = timer()
    logger.debug('Starting Reinforce training!')
    ...
    params_training = {
        ...
        'rl':{
            'sampling': params.get('RL_SAMPLING', 'multinomial'),
    ...
    if params['RL_SAMPLING'] == 'multinomial':
        sampler = 'multinomial'
    elif params['RL_SAMPLING'] == 'beam_search':
        sampler = BeamSearchEnsemble([nmt_model],
                                     dataset,
                                     params_prediction,
                                     verbose=verbose)

    else:
        raise NotImplementedError('Unknown RL_SAMPLING strategy: %s' % str(params['RL_SAMPLING']))


    data_gen = Data_Batch_Generator('train',
                                    nmt_model,
                                    dataset,
                                    params['EPISODES'],
                                    batch_size=1,
                                    predict=False,
                                    random_samples=-1).generator()
    reinforce_trainer = ReinforceTrainer(nmt_model, dataset, sampler, data_gen, params_training=params_training, params_prediction=params_prediction, verbose=params['VERBOSE'])
...
def train_model(params, load_dataset=None):
    ...
    params_training = {'n_epochs': params['MAX_EPOCH'],
                       ...
                       'mrt':{
                            'sampling': params.get('MRT_SAMPLING', 'multinomial'),
                            'mrt_loss': params['MRT_LOSS'],
                            'mrt_samples': params['MRT_SAMPLES'],
                            'loss_model_index': params['LOSS_MODEL_INDEX'],
                            'episodes': params['EPISODES']
                            }
                       }
    if parameters.get('TRAINING_OBJECTIVE', 'mle').lower() == 'mrt':
        ...
        nmt_model.trainMinimumRiskNet(dataset, params_training, params_prediction)
    else:
        nmt_model.trainNet(dataset, params_training)
...
def train_model_online(params, source_filename, target_filename, models_path=None, dataset=None, stored_hypotheses_filename=None,
    ...
    :param stored_hypotheses_filename: Path for storing the model hypotheses
    ...
    # Open new data
    target_lines = file2list(target_filename, stripfile=True)
    source_lines = file2list(source_filename, stripfile=True)
    ...
    [saveModel(nmt_model,
               -1,
               path=params.get('STORE_PATH', 'retrained_model'),
               full_path=True) for nmt_model in models]
